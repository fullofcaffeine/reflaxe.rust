...
- `cargo hx dev` watches, recompiles, and runs after each source change.
- `cargo hx test` runs generated Rust tests once.
- `cargo hx build --release` verifies the production build path.
...
`cargo hx dev` selects the same profile/HXML as the one-shot commands and reuses the shipped
incremental watcher. The lower-level script remains available for unusual custom automation.
