...
class Digest {
	...
	public static function sign(data:haxe.io.Bytes, privKey:Key, alg:DigestAlgorithm):haxe.io.Bytes {
		return untyped __rust__("hxrt::ssl::digest_sign({0}.borrow().as_slice(), &{1}, {2}.as_str())", data, untyped privKey.handle, alg);
	}

	public static function verify(data:haxe.io.Bytes, signature:haxe.io.Bytes, pubKey:Key, alg:DigestAlgorithm):Bool {
		return untyped __rust__("hxrt::ssl::digest_verify({0}.borrow().as_slice(), {1}.borrow().as_slice(), &{2}, {3}.as_str())", data, signature,
			untyped pubKey.handle, alg);
	}
}
