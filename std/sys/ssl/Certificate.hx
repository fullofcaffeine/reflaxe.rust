...
class Certificate {
	...
	private function get_altNames():Array<String> {
		...
		return
			untyped __rust__("hxrt::array::Array::<hxrt::string::HxString>::from_vec({0}.borrow().alt_names().into_iter().map(hxrt::string::HxString::from).collect::<Vec<hxrt::string::HxString>>())",
			handle);
		#else
