...
class ElasticThreadPool implements IThreadPool {
	...
	**/
	public function new(maxThreadsCount: Int, threadTimeout: Float = 60): Void {
		if (maxThreadsCount < 1) throw new ThreadPoolException("ElasticThreadPool needs maxThreadsCount to be at least 1.");
	...
	**/
	public function run(task: () -> Void): Void {
		...
		var deadWorker = null;
		for (worker in pool) {
			if (deadWorker == null && worker.dead) deadWorker = worker;
			if (worker.task == null) {
				submitted = true;
...
private class Worker {
	...
	public static function wakeup(worker: Worker, task: () -> Void) {
		...
		if (worker.dead) Worker.start(worker);
		worker.task = task;
	...
	static function loop(worker: Worker) {
		try {
			...
			if (worker.task != null) Worker.start(worker) else worker.dead = true;
			worker.deathMutex.release();
		} catch (e) {
			worker.task = null;
			Worker.start(worker);
