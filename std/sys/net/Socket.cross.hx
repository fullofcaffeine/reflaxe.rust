...
class Socket {
	...
	**/
	...
	public function shutdown(read:Bool, write:Bool):Void {
		untyped __rust__("{0}.borrow_mut().shutdown({1} as bool, {2} as bool)", handle, read, write);
	}
	...
	public function waitForRead():Void {
		untyped __rust__("{ let _ = hxrt::net::socket_select(vec![{0}.clone()], vec![], vec![], Some(-1.0)); }", handle);
	}
