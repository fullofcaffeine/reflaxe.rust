...
- Use `/Users/fullofcaffeine/workspace/code/haxe.elixir.reference` for patterns/APIs we previously used for the Haxe→Elixir target.
- Use `/Users/fullofcaffeine/workspace/code/haxe.elixir.codex` for the original Haxe→Elixir compiler implementation (**read-only; do not modify anything in that repo**).
