...
- Polymorphic field-update gotcha: a base-typed value is a Rust trait object and cannot borrow a concrete child's physical field directly. Lower compound assignments and numeric prefix/postfix updates through the same generated typed field getter/setter methods used by ordinary polymorphic reads/writes; evaluate the receiver and RHS once, and preserve Haxe's assigned/old/new expression result. Do not add a runtime field registry or dynamic handle for this compiler-known contract.
- Static field-update gotcha: mutable static storage is hidden behind generated lazy-cell getter/setter functions, so a static read is not a Rust lvalue. Lower compound assignments and numeric prefix/postfix updates as typed getter → direct operation → typed setter, evaluate the RHS once, and preserve assigned/old/new Haxe expression results. Reuse the existing static storage contract; do not add another global registry or runtime API.
...
- Array element-update gotcha: `Array<T>` indexing lowers through typed get/set methods and is not a Rust place expression. For Copy-like numeric compound or prefix/postfix updates, evaluate the array then index once, use the existing array storage API, and preserve Haxe's assigned/old/new expression result. Do not add an array-specific runtime update helper for this compiler-known operation.
