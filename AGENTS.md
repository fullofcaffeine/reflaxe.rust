...
- Native facade helper policy (strict): `std/rust/native/*.rs` modules are not `hxrt`, but they are still handwritten Rust shipped with generated crates and must not become a shadow runtime. Prefer compiler lowering when the operation is a pure, closed transformation from typed AST, metadata, literals, or existing Rust primitives. A native helper is allowed only for a narrow typed `rust.*` / metal facade that owns or safely wraps a real Rust type/resource the current Haxe/compiler surface cannot express cleanly without raw snippets, dynamic handles, layout assumptions, or noisy generated Rust. Valid reasons include private-field/native-layout wrappers, RAII/resource lifecycle, partial moves, pipe/stream/socket ownership, lifetimes/HRTB, const generics, trait impl bodies, macro-heavy setup, contained `unsafe`, or narrow typed validation at a native boundary. Forbidden helper growth includes `hxrt` dependencies, `Dynamic`/`Any`, type-erased registries, broad handles, reflection-like dispatch, portable `sys.*` semantics under `rust.*`, generic platform abstraction, allocation-heavy adapters where direct Rust values are available, or catch-all helper modules that collect unrelated behavior. Every new or expanded helper must document its owning Haxe facade, why compiler lowering is insufficient today, its classification (`permanent-native-facade`, `lowering-candidate`, or `experimental-scaffold`), allowed dependencies/imports, generated call-site evidence, and no-hxrt evidence.
