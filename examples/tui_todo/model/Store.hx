...
class Store {
	...
	public function save(): Void {
		...
		for (t in tasks) list.push(t.toData());
		...
		if (hadOld) FileSystem.rename(path, backup);
	...
	static function decodeTaskListV0(raw: Dynamic): Array<TaskDataV1> {
		...
		for (entry in legacyTasks) {
			...
			var createdAt = readOptionalInt(entry, "createdAt");
			if (createdAt == null) createdAt = now;
			var updatedAt = readOptionalInt(entry, "updatedAt");
