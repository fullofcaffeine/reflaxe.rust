...
| Rust floor and dependency resolution | Resolver 3 and generated `rust-version = "1.96.0"` now share one structured policy with committed application locks and locked CI/release builds. Two empty-Cargo-home passes protect deterministic locks and normalized metadata for minimal, portable, systems/TLS, async-feature, and metal crates; exact-minimum and current-stable CI check the reviewed baseline and archive it, while a dependency mutation proves an unsupported Rust requirement fails. | `haxe_rust-p6hs.11` complete |
...
4. `haxe_rust-p6hs.11` — fresh MSRV resolution and lock policy (implemented); `.12` and `.13` — CI
   input identity and vendor/license governance.
