...
| `reflaxe.std.*` | `portable contract` | `test/semantic_diff/portable_option_result_basics`, `test/snapshot/reflaxe_std_option_result`, `test/snapshot/rust_reflaxe_std_adapters` | Shared portable idiom layer. v1 currently starts with `Option` / `Result`. |
