...
| `std/rust/native/current_process_tools.rs` | `rust.process.CurrentProcess`, `CurrentProcessError` | `permanent-native-facade` | Keep as a narrow owner of temporary standard-stream locks, exact binary writes, flushes, checked bytes, and diverging process exit. | `test/positive/metal_no_hxrt_current_process` proves exact bytes and statuses. Its policy case requires no `hxrt`, direct Rust standard IO, `rust_forbid_unsafe`, rustfmt, Cargo, Clippy, and exact runtime output. |
