...
| Socket/TLS handles | `hxrt.net.SocketHandle`, `hxrt.ssl.*Handle`, `rust.net.NativeTcp`, `rust.net.TcpListener`, `rust.net.TcpStream`, `rust.net.NativeUdp`, `rust.net.UdpSocket`, `rust.net.SocketError` | Runtime-owned portable TCP/UDP/TLS handles for `sys.*`; Rust-native blocking TCP and UDP wrappers over direct `std::net` for narrow metal slices; typed opt-in TCP/UDP error records | partial | `test/snapshot/sys_ssl_sni`, sys.net/sys.ssl sweeps/examples; `std/rust/net/NativeTcp.hx`; `std/rust/net/TcpListener.hx`; `std/rust/net/TcpStream.hx`; `std/rust/net/NativeUdp.hx`; `std/rust/net/UdpSocket.hx`; `std/rust/net/SocketError.hx`; `test/positive/metal_no_hxrt_native_tcp`; `test/positive/metal_no_hxrt_native_udp`; `test/positive/metal_no_hxrt_socket_error`; `scripts/ci/check-metal-policy.sh` native TCP/UDP/socket-error output-shape gates | Current Rust-native socket facades are localhost blocking TCP and localhost blocking UDP with opt-in invalid-input/IO/UTF-8 `SocketError` categories. Arbitrary host/address APIs, live stream adapters, async networking, TLS setup, portable `sys.net` parity, byte-buffer datagram APIs, and richer platform-specific error categories remain future work. |
