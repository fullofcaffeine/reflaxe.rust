...
| Socket/TLS handles | `hxrt.net.SocketHandle`, `hxrt.ssl.*Handle`, `rust.net.NativeTcp`, `rust.net.TcpListener`, `rust.net.TcpStream` | Runtime-owned portable TCP/UDP/TLS handles for `sys.*`; Rust-native blocking TCP wrappers over direct `std::net` for the first metal slice | partial | `test/snapshot/sys_ssl_sni`, sys.net/sys.ssl sweeps/examples; `std/rust/net/NativeTcp.hx`; `std/rust/net/TcpListener.hx`; `std/rust/net/TcpStream.hx`; `test/positive/metal_no_hxrt_native_tcp`; `scripts/ci/check-metal-policy.sh` native TCP output-shape gate | Current Rust-native socket facade is localhost blocking TCP only. Arbitrary host/address APIs, UDP, live stream adapters, async networking, TLS setup, portable `sys.net` parity, and richer error categories remain future work. |
