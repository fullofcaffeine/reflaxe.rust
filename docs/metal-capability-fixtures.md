...
| Rust-native systems facades | `docs/metal-systems-facades-roadmap.md`, typed `rust.fs`, `rust.process`, and `rust.net` facades, and their no-hxrt fixtures | TLS/DB, reusable stream handles, detached handles, async processes, shell fallback, arbitrary hosts, DNS, and external networking remain future work | positive/negative + output-shape + cargo check/run |
...
| `haxe_rust-x6xb` | Current-process binary IO facade, safe-only generated crate, and exact subprocess fixture. |
