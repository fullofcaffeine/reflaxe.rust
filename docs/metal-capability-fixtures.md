...
| Rust-native systems facades | `docs/metal-systems-facades-roadmap.md`, `std/rust/fs/NativeFiles.hx`, `std/rust/process/NativeCommands.hx`, `std/rust/process/CommandOutput.hx`, `test/positive/metal_no_hxrt_native_file`, `test/negative/metal_fs_raw_escape`, `test/positive/metal_no_hxrt_native_process`, `test/positive/metal_no_hxrt_command_output`, `test/positive/metal_no_hxrt_command_cwd`, `test/negative/metal_process_raw_escape` | Socket/TLS/DB after process/file patterns are proven; process env, stdin, live handles, and async remain future work | positive/negative + output-shape + cargo check/run |
