...
- Contract reports (`-D rust_contract_report`) include source-text native-import hits, user-source
  typed native usage (`nativeImportHitsTyped`), a portability marker, admitted portable facade
  surfaces, and selected native representation decisions.
