...
| JSON/schema validation | `json_schema_validate` | `portable-first` | Existing fixture: `test/perf/json_schema_validate` | Reuse `json` thresholds and artifact flow. | Parse dynamic JSON at the boundary, validate into typed structures, and expose avoidable runtime-shape churn. |
