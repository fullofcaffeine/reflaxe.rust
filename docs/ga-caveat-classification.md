...
| `sys.Http` targeted local-server plus smoke-backed confidence | `docs/feature-support-matrix.md`, `docs/semantic-confidence-summary.md`, `test/semantic_diff/sys_http_callback_contract`, `test/snapshot/sys_http_smoke`, Tier1/Tier2 sweeps | `explicit defer` | The current proof depth combines targeted callback/status/body/error parity with snapshot-backed request assembly and generated-shape smoke. That is enough to keep `sys.Http` in the supported portable surface, but not enough for blanket host/network semantic-parity language. | Leave `sys.Http` qualified as supported with targeted local-server plus smoke-backed proof depth rather than full host/network closure. |
