...
## Can a Rust compiler error point back to my Haxe code?

The compiler now emits `rust-source-map.json` with every generated crate. It records exact,
content-hashed Rust byte ranges and maps them either to a Haxe source position or to an explicit
compiler-generated reason. Lookup requires the complete generated filename and matching file bytes;
it does not guess from a basename or stale line number.

This is the mapping foundation, not yet a claim that every Cargo/rustc error is automatically
rewritten into a polished Haxe diagnostic. Also, running rustfmt changes the generated bytes and
intentionally invalidates the old map. See [Rust-to-Haxe source maps](rust-source-maps.md) for the
schema, exact-lookup rules, and current integration boundary.
...
fields. The compiler checks every declared field, including an `@:optional` field omitted from the
literal, before the record is created or converted to `Dynamic`. This also prevents runtime-name
reflection and writes through a `Dynamic` alias from bypassing the rule after the field declaration is
no longer visible. A direct write points to the exact stored value; an omitted field points to the
anonymous value whose complete shape is unsupported. Store an owned `T` value instead. Supporting a
borrowed field would require a new guarded-read API that ties the returned reference to the record's
read lock; the current value-returning field API cannot promise that lifetime safely.
