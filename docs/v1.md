...
- Typed catch now follows subclass chains for the emitted non-generic class-hierarchy slice (see `test/semantic_diff/typed_catch_subclass`).
  Remaining exact-type caveats are narrower: interface-typed catches and other paths that do not carry emitted subclass metadata still fall back to exact `Dynamic.downcast` behavior.
