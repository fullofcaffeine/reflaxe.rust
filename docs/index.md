...
- [Metal systems facades roadmap](metal-systems-facades-roadmap.md): shipped file/path and owned-command/process-output/cwd/env set-remove-clear/cwd+env/stdin-input/stdin+cwd+env/command-spec/command-error/command-child lifecycle slices for Rust-native systems handles distinct from portable `sys.*` semantics.
