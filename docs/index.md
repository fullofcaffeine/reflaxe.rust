...
- [Rust-to-Haxe source maps](rust-source-maps.md) and [version-1 schema](schemas/rust-source-map-v1.schema.json): deterministic, path-private generated byte ranges, typed origins, exact content-hash lookup, and the current rustc-integration boundary.
- [Rust representation decision model](rust-representation-plan.md) and [decision component schema](schemas/rust-representation-decision-v1.schema.json): the closed typed vocabulary for storage shape, null encoding, ownership/reuse, semantic runtime reasons, contextual bounds, and exact source-private Haxe origins.
- [Typed Rust IR and raw-authority closure](rust-raw-authority.md) and [generated inventory](rust-raw-authority-inventory.json): zero compiler-owned raw sites, the two exact author-controlled escape boundaries, and the deterministic growth guard.
