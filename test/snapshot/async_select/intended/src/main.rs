...
fn quick(label: String) -> hxrt::async_::HxFuture<String> {
...
fn slow(label: String) -> hxrt::async_::HxFuture<String> {
    return Box::pin(async move {
        hxrt::async_::sleep_ms(20).await;
        return label;
    });
}
...
fn flow() -> hxrt::async_::HxFuture<String> {
    return Box::pin(async move {
        let first: String = hxrt::async_::select_first(
            quick(String::from("left-fast")),
            slow(String::from("right-slow")),
        )
        .await;
        let second: String = hxrt::async_::select_first(
            slow(String::from("left-slow")),
            quick(String::from("right-fast")),
        )
        .await;
