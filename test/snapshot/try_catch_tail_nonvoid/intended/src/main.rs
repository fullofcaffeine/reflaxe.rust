...
fn decode(raw: hxrt::string::HxString) -> i32 {
    match hxrt::exception::catch_unwind(|| {
        if raw == hxrt::string::HxString::from("bad") {
            hxrt::exception::throw(hxrt::dynamic::from(hxrt::string::HxString::from("invalid")));
        }
        return hxrt::string::len(raw.as_str());
    }) {
        Ok(__hx_ok) => __hx_ok,
        Err(__hx_ex) => {
            let _ = __hx_ex;
