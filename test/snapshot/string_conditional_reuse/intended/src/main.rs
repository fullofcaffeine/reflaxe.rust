...
fn render(
    ...
    let final_text: hxrt::string::HxString = hxrt::string::HxString::from(if use_original {
        hxrt::string::HxString::from(message.clone())
    } else {
        hxrt::string::HxString::from("fallback")
    });
    let last_text: hxrt::string::HxString = hxrt::string::HxString::from(if use_last {
        hxrt::string::HxString::from(message.clone())
    } else {
        hxrt::string::HxString::from("last")
    });
    let notification: hxrt::string::HxString =
        hxrt::string::HxString::from(if hxrt::string::len(final_text.as_str()) > 0 {
            hxrt::string::HxString::from(message.clone())
        } else {
    ...
    return hxrt::string::HxString::from(format!(
        "{}{}{}{}{}{}{}",
        &final_text, "|", &last_text, "|", &notification, "|", &message
    ));
}

fn main() {
    crate::sys::Sys::println(hxrt::dynamic::from(render(
