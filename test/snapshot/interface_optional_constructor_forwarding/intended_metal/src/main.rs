...
pub(crate) fn __hx_is_subtype_type_id(actual: u32, expected: u32) -> bool {
    ...
    match actual {
        0xfddc42d8u32 => expected == 0x4d5f7783u32,
...
fn forward(
    channel: crate::HxRc<dyn crate::i_channel::IChannel + Send + Sync>,
) -> crate::HxRef<crate::channel_owner::ChannelOwner> {
    return crate::channel_owner::ChannelOwner::new(crate::HxDynRef::new(channel.clone()));
}

fn main() {
    let channel: crate::HxRc<dyn crate::i_channel::IChannel + Send + Sync> = {
        let __tmp = crate::channel::Channel::new();
        let __up: crate::HxRc<dyn crate::i_channel::IChannel + Send + Sync> =
            __tmp.as_arc_opt().unwrap().clone();
        __up
    };
    crate::sys::Sys::println(hxrt::dynamic::from(
        crate::channel_owner::ChannelOwner::label(&*forward(channel.clone())),
    ));
}
