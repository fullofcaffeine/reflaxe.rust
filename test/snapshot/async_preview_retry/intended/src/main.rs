...
fn next_value(value: i32) -> hxrt::async_::HxFuture<i32> {
...
fn compute() -> hxrt::async_::HxFuture<String> {
    return Box::pin(async move {
        let a: i32 = next_value(40).await;
        let b: i32 = next_value(a).await;
        return format!("{}{}", "value=", hxrt::dynamic::from(b).to_haxe_string());
    });
}

fn main() {
    crate::sys::Sys::println(hxrt::dynamic::from(hxrt::async_::block_on(compute())));
}
