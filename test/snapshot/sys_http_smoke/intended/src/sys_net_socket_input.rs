// Generated by reflaxe.rust

pub const __HX_TYPE_ID: u32 = 0x45aaab62u32;

#[derive(Debug)]

pub struct SocketInput {
    pub big_endian: bool,
    handle: crate::HxRef<hxrt::net::SocketHandle>,
}

impl SocketInput {
    pub fn new(
        handle: crate::HxRef<hxrt::net::SocketHandle>,
    ) -> crate::HxRef<crate::sys_net_socket_input::SocketInput> {
        let self_: crate::HxRef<crate::sys_net_socket_input::SocketInput> =
            crate::HxRef::new(SocketInput {
                big_endian: false,
                handle: handle,
            });
        return self_;
    }

    pub fn read_byte(self_: &crate::HxRefCell<SocketInput>) -> i32 {
        let __hx_this: crate::HxRef<crate::sys_net_socket_input::SocketInput> = self_.self_ref();
        let b: crate::HxRef<hxrt::bytes::Bytes> =
            crate::HxRef::new(hxrt::bytes::Bytes::alloc(1 as usize));
        let n: i32 =
            crate::sys_net_socket_input::SocketInput::read_bytes(&*__hx_this, b.clone(), 0, 1);
        if n == 0 {
            hxrt::exception::throw(hxrt::dynamic::from(crate::haxe_io_eof::Eof::new()));
        }
        return b.borrow().get(0);
    }

    pub fn read_bytes(
        self_: &crate::HxRefCell<SocketInput>,
        s: crate::HxRef<hxrt::bytes::Bytes>,
        pos: i32,
        len: i32,
    ) -> i32 {
        let __hx_this: crate::HxRef<crate::sys_net_socket_input::SocketInput> = self_.self_ref();
        if pos < 0 || len < 0 || pos + len > s.borrow().length() {
            hxrt::exception::throw(hxrt::dynamic::from(hxrt::io::Error::OutsideBounds));
        }
        if len == 0 {
            return 0;
        }
        let out: i32 = {
            let mut buf = vec![0u8; len as usize];
            let n: i32 = {
                let __b = __hx_this.borrow();
                __b.handle.clone()
            }
            .borrow_mut()
            .read_stream(buf.as_mut_slice());
            if n == -1i32 {
                0i32
            } else {
                hxrt::bytes::write_from_slice(&s, pos, &buf[0..(n as usize)]);
                n
            }
        };
        if out == 0 {
            hxrt::exception::throw(hxrt::dynamic::from(crate::haxe_io_eof::Eof::new()));
        }
        return out;
    }

    pub fn close(self_: &crate::HxRefCell<SocketInput>) {
        let __hx_this: crate::HxRef<crate::sys_net_socket_input::SocketInput> = self_.self_ref();
        crate::sys_net_socket_input::SocketInput::__hx_super_haxe_io_input_close(&*self_);
        {
            let __b = __hx_this.borrow();
            __b.handle.clone()
        }
        .borrow_mut()
        .close();
    }

    pub fn set_big_endian(self_: &crate::HxRefCell<SocketInput>, b: bool) -> bool {
        let __hx_this: crate::HxRef<crate::sys_net_socket_input::SocketInput> = self_.self_ref();
        {
            let __tmp = b;
            __hx_this.borrow_mut().big_endian = __tmp;
            __tmp
        };
        return b;
    }

    pub fn read_all(
        self_: &crate::HxRefCell<SocketInput>,
        bufsize: Option<i32>,
    ) -> crate::HxRef<hxrt::bytes::Bytes> {
        let __hx_this: crate::HxRef<crate::sys_net_socket_input::SocketInput> = self_.self_ref();
        let mut bs: Option<i32> = bufsize;
        if bs.is_none() {
            {
                let __tmp = 16384;
                bs = Some(__tmp);
                __tmp
            };
        }
        let bufsize_2: i32 = {
            let __hx_opt = bs.clone();
            match &__hx_opt {
                Some(__v) => __v.clone(),
                None => hxrt::exception::throw(hxrt::dynamic::from(String::from("Null Access"))),
            }
        };
        let buf: crate::HxRef<hxrt::bytes::Bytes> =
            crate::HxRef::new(hxrt::bytes::Bytes::alloc(bufsize_2 as usize));
        let total: crate::HxRef<crate::haxe_io_bytes_buffer::BytesBuffer> =
            crate::haxe_io_bytes_buffer::BytesBuffer::new();
        match hxrt::exception::catch_unwind(|| loop {
            let len: i32 = crate::sys_net_socket_input::SocketInput::read_bytes(
                &*__hx_this,
                buf.clone(),
                0,
                bufsize_2,
            );
            if len == 0 {
                hxrt::exception::throw(hxrt::dynamic::from(hxrt::io::Error::Blocked));
            }
            crate::haxe_io_bytes_buffer::BytesBuffer::add_bytes(&*total, buf.clone(), 0, len);
        }) {
            Ok(__hx_ok) => __hx_ok,
            Err(__hx_ex) => match __hx_ex.downcast::<crate::HxRef<crate::haxe_io_eof::Eof>>() {
                Ok(_) => {}
                Err(__hx_ex) => hxrt::exception::rethrow(__hx_ex),
            },
        };
        return crate::haxe_io_bytes_buffer::BytesBuffer::get_bytes(&*total);
    }

    pub fn read_full_bytes(
        self_: &crate::HxRefCell<SocketInput>,
        s: crate::HxRef<hxrt::bytes::Bytes>,
        pos: i32,
        len: i32,
    ) {
        let __hx_this: crate::HxRef<crate::sys_net_socket_input::SocketInput> = self_.self_ref();
        let mut p: i32 = pos;
        let mut l: i32 = len;
        while l > 0 {
            let k: i32 =
                crate::sys_net_socket_input::SocketInput::read_bytes(&*__hx_this, s.clone(), p, l);
            if k == 0 {
                hxrt::exception::throw(hxrt::dynamic::from(hxrt::io::Error::Blocked));
            }
            {
                p = p + k;
                p
            };
            {
                l = l - k;
                l
            };
        }
    }

    pub fn read(
        self_: &crate::HxRefCell<SocketInput>,
        nbytes: i32,
    ) -> crate::HxRef<hxrt::bytes::Bytes> {
        let __hx_this: crate::HxRef<crate::sys_net_socket_input::SocketInput> = self_.self_ref();
        let mut remaining: i32 = nbytes;
        let s: crate::HxRef<hxrt::bytes::Bytes> =
            crate::HxRef::new(hxrt::bytes::Bytes::alloc(remaining as usize));
        let mut p: i32 = 0;
        while remaining > 0 {
            let k: i32 = crate::sys_net_socket_input::SocketInput::read_bytes(
                &*__hx_this,
                s.clone(),
                p,
                remaining,
            );
            if k == 0 {
                hxrt::exception::throw(hxrt::dynamic::from(hxrt::io::Error::Blocked));
            }
            {
                p = p + k;
                p
            };
            {
                remaining = remaining - k;
                remaining
            };
        }
        return s;
    }

    pub fn read_until(self_: &crate::HxRefCell<SocketInput>, end: i32) -> hxrt::string::HxString {
        let __hx_this: crate::HxRef<crate::sys_net_socket_input::SocketInput> = self_.self_ref();
        let buf: crate::HxRef<crate::haxe_io_bytes_buffer::BytesBuffer> =
            crate::haxe_io_bytes_buffer::BytesBuffer::new();
        loop {
            let last: i32 = crate::sys_net_socket_input::SocketInput::read_byte(&*__hx_this);
            if last == end {
                break;
            }
            crate::haxe_io_bytes_buffer::BytesBuffer::add_byte(&*buf, last);
        }
        return hxrt::string::HxString::from(
            crate::haxe_io_bytes_buffer::BytesBuffer::get_bytes(&*buf)
                .borrow()
                .to_string(),
        );
    }

    pub fn read_line(self_: &crate::HxRefCell<SocketInput>) -> hxrt::string::HxString {
        let __hx_this: crate::HxRef<crate::sys_net_socket_input::SocketInput> = self_.self_ref();
        let buf: crate::HxRef<crate::haxe_io_bytes_buffer::BytesBuffer> =
            crate::haxe_io_bytes_buffer::BytesBuffer::new();
        match hxrt::exception::catch_unwind(|| loop {
            let last: i32 = crate::sys_net_socket_input::SocketInput::read_byte(&*__hx_this);
            if last == 10 {
                break;
            }
            crate::haxe_io_bytes_buffer::BytesBuffer::add_byte(&*buf, last);
        }) {
            Ok(__hx_ok) => __hx_ok,
            Err(__hx_ex) => match __hx_ex.downcast::<crate::HxRef<crate::haxe_io_eof::Eof>>() {
                Ok(__hx_box) => {
                    let e: crate::HxRef<crate::haxe_io_eof::Eof> = *__hx_box;
                    let bytes: crate::HxRef<hxrt::bytes::Bytes> =
                        crate::haxe_io_bytes_buffer::BytesBuffer::get_bytes(&*buf);
                    if bytes.borrow().length() == 0 {
                        hxrt::exception::throw(hxrt::dynamic::from(e));
                    }
                    return hxrt::string::HxString::from(bytes.borrow().to_string());
                }
                Err(__hx_ex) => hxrt::exception::rethrow(__hx_ex),
            },
        };
        let mut bytes_2: crate::HxRef<hxrt::bytes::Bytes> =
            crate::haxe_io_bytes_buffer::BytesBuffer::get_bytes(&*buf);
        let bytes_len: i32 = bytes_2.borrow().length();
        if bytes_len > 0 {
            let last_byte: i32 = bytes_2.borrow().get(bytes_len - 1);
            if last_byte == 13 {
                let trimmed: crate::HxRef<hxrt::bytes::Bytes> =
                    crate::HxRef::new(bytes_2.borrow().sub(0, bytes_len - 1));
                bytes_2 = trimmed;
            }
        }
        return hxrt::string::HxString::from(bytes_2.borrow().to_string());
    }

    pub fn read_float(self_: &crate::HxRefCell<SocketInput>) -> f64 {
        let __hx_this: crate::HxRef<crate::sys_net_socket_input::SocketInput> = self_.self_ref();
        return crate::haxe_io_fp_helper::FPHelper::i32_to_float(
            crate::sys_net_socket_input::SocketInput::read_int32(&*__hx_this),
        );
    }

    pub fn read_double(self_: &crate::HxRefCell<SocketInput>) -> f64 {
        let __hx_this: crate::HxRef<crate::sys_net_socket_input::SocketInput> = self_.self_ref();
        let i1: i32 = crate::sys_net_socket_input::SocketInput::read_int32(&*__hx_this);
        let i2: i32 = crate::sys_net_socket_input::SocketInput::read_int32(&*__hx_this);
        return if {
            let __b = __hx_this.borrow();
            __b.big_endian
        } {
            crate::haxe_io_fp_helper::FPHelper::i64_to_double(i2, i1)
        } else {
            crate::haxe_io_fp_helper::FPHelper::i64_to_double(i1, i2)
        };
    }

    pub fn read_int8(self_: &crate::HxRefCell<SocketInput>) -> i32 {
        let __hx_this: crate::HxRef<crate::sys_net_socket_input::SocketInput> = self_.self_ref();
        let n: i32 = crate::sys_net_socket_input::SocketInput::read_byte(&*__hx_this);
        if n >= 128 {
            return n - 256;
        }
        return n;
    }

    pub fn read_int16(self_: &crate::HxRefCell<SocketInput>) -> i32 {
        let __hx_this: crate::HxRef<crate::sys_net_socket_input::SocketInput> = self_.self_ref();
        let ch1: i32 = crate::sys_net_socket_input::SocketInput::read_byte(&*__hx_this);
        let ch2: i32 = crate::sys_net_socket_input::SocketInput::read_byte(&*__hx_this);
        let n: i32 = if {
            let __b = __hx_this.borrow();
            __b.big_endian
        } {
            ch1 << 8 | ch2
        } else {
            ch1 | ch2 << 8
        };
        if n >= 32768 {
            return n - 65536;
        }
        return n;
    }

    pub fn read_u_int16(self_: &crate::HxRefCell<SocketInput>) -> i32 {
        let __hx_this: crate::HxRef<crate::sys_net_socket_input::SocketInput> = self_.self_ref();
        let ch1: i32 = crate::sys_net_socket_input::SocketInput::read_byte(&*__hx_this);
        let ch2: i32 = crate::sys_net_socket_input::SocketInput::read_byte(&*__hx_this);
        return if {
            let __b = __hx_this.borrow();
            __b.big_endian
        } {
            ch1 << 8 | ch2
        } else {
            ch1 | ch2 << 8
        };
    }

    pub fn read_int24(self_: &crate::HxRefCell<SocketInput>) -> i32 {
        let __hx_this: crate::HxRef<crate::sys_net_socket_input::SocketInput> = self_.self_ref();
        let ch1: i32 = crate::sys_net_socket_input::SocketInput::read_byte(&*__hx_this);
        let ch2: i32 = crate::sys_net_socket_input::SocketInput::read_byte(&*__hx_this);
        let ch3: i32 = crate::sys_net_socket_input::SocketInput::read_byte(&*__hx_this);
        let n: i32 = if {
            let __b = __hx_this.borrow();
            __b.big_endian
        } {
            ch1 << 16 | ch2 << 8 | ch3
        } else {
            ch1 | ch2 << 8 | ch3 << 16
        };
        if n >= 8388608 {
            return n - 16777216;
        }
        return n;
    }

    pub fn read_u_int24(self_: &crate::HxRefCell<SocketInput>) -> i32 {
        let __hx_this: crate::HxRef<crate::sys_net_socket_input::SocketInput> = self_.self_ref();
        let ch1: i32 = crate::sys_net_socket_input::SocketInput::read_byte(&*__hx_this);
        let ch2: i32 = crate::sys_net_socket_input::SocketInput::read_byte(&*__hx_this);
        let ch3: i32 = crate::sys_net_socket_input::SocketInput::read_byte(&*__hx_this);
        return if {
            let __b = __hx_this.borrow();
            __b.big_endian
        } {
            ch1 << 16 | ch2 << 8 | ch3
        } else {
            ch1 | ch2 << 8 | ch3 << 16
        };
    }

    pub fn read_int32(self_: &crate::HxRefCell<SocketInput>) -> i32 {
        let __hx_this: crate::HxRef<crate::sys_net_socket_input::SocketInput> = self_.self_ref();
        let ch1: i32 = crate::sys_net_socket_input::SocketInput::read_byte(&*__hx_this);
        let ch2: i32 = crate::sys_net_socket_input::SocketInput::read_byte(&*__hx_this);
        let ch3: i32 = crate::sys_net_socket_input::SocketInput::read_byte(&*__hx_this);
        let ch4: i32 = crate::sys_net_socket_input::SocketInput::read_byte(&*__hx_this);
        return if {
            let __b = __hx_this.borrow();
            __b.big_endian
        } {
            ch4 | ch3 << 8 | ch2 << 16 | ch1 << 24
        } else {
            ch1 | ch2 << 8 | ch3 << 16 | ch4 << 24
        };
    }

    pub fn read_string(
        self_: &crate::HxRefCell<SocketInput>,
        len: i32,
        encoding: Option<crate::haxe_io_encoding::Encoding>,
    ) -> hxrt::string::HxString {
        let __hx_this: crate::HxRef<crate::sys_net_socket_input::SocketInput> = self_.self_ref();
        let b: crate::HxRef<hxrt::bytes::Bytes> =
            crate::HxRef::new(hxrt::bytes::Bytes::alloc(len as usize));
        crate::sys_net_socket_input::SocketInput::read_full_bytes(&*__hx_this, b.clone(), 0, len);
        return hxrt::string::HxString::from({
            let _ = encoding;
            b.borrow().get_string(0, len)
        });
    }

    fn __hx_super_haxe_io_input_close(_self_: &crate::HxRefCell<SocketInput>) {}
}

impl crate::haxe_io_input::InputTrait for crate::HxRefCell<SocketInput> {
    fn __hx_get_big_endian(&self) -> bool {
        self.borrow().big_endian
    }
    fn __hx_set_big_endian(&self, v: bool) {
        self.borrow_mut().big_endian = v;
    }
    fn close(&self) -> () {
        SocketInput::close(self)
    }
    fn read(&self, nbytes: i32) -> crate::HxRef<hxrt::bytes::Bytes> {
        SocketInput::read(self, nbytes)
    }
    fn read_all(&self, bufsize: Option<i32>) -> crate::HxRef<hxrt::bytes::Bytes> {
        SocketInput::read_all(self, bufsize)
    }
    fn read_byte(&self) -> i32 {
        SocketInput::read_byte(self)
    }
    fn read_bytes(&self, s: crate::HxRef<hxrt::bytes::Bytes>, pos: i32, len: i32) -> i32 {
        SocketInput::read_bytes(self, s, pos, len)
    }
    fn read_double(&self) -> f64 {
        SocketInput::read_double(self)
    }
    fn read_float(&self) -> f64 {
        SocketInput::read_float(self)
    }
    fn read_full_bytes(&self, s: crate::HxRef<hxrt::bytes::Bytes>, pos: i32, len: i32) -> () {
        SocketInput::read_full_bytes(self, s, pos, len)
    }
    fn read_int16(&self) -> i32 {
        SocketInput::read_int16(self)
    }
    fn read_int24(&self) -> i32 {
        SocketInput::read_int24(self)
    }
    fn read_int32(&self) -> i32 {
        SocketInput::read_int32(self)
    }
    fn read_int8(&self) -> i32 {
        SocketInput::read_int8(self)
    }
    fn read_line(&self) -> hxrt::string::HxString {
        SocketInput::read_line(self)
    }
    fn read_string(
        &self,
        len: i32,
        encoding: Option<crate::haxe_io_encoding::Encoding>,
    ) -> hxrt::string::HxString {
        SocketInput::read_string(self, len, encoding)
    }
    fn read_u_int16(&self) -> i32 {
        SocketInput::read_u_int16(self)
    }
    fn read_u_int24(&self) -> i32 {
        SocketInput::read_u_int24(self)
    }
    fn read_until(&self, end: i32) -> hxrt::string::HxString {
        SocketInput::read_until(self, end)
    }
    fn set_big_endian(&self, b: bool) -> bool {
        SocketInput::set_big_endian(self, b)
    }
    fn __hx_type_id(&self) -> u32 {
        crate::sys_net_socket_input::__HX_TYPE_ID
    }
}
