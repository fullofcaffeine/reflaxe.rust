...
[![Version](https://img.shields.io/badge/version-0.71.0-blue)](https://github.com/fullofcaffeine/reflaxe.rust/releases)
...
- Rust-native systems facades are growing beside portable `sys.*` APIs. Current slices include
  ...
  narrow owned-command process subset with `rust.process.CommandOutput` and explicit working
  directory/environment set-remove-clear/cwd+env/stdin-input/stdin+cwd+env support; neither replaces
  portable `sys.io.File` / `sys.io.Process`.
