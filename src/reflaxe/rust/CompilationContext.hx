...
class CompilationContext {
	...
	var sourceMapFiles:Array<RustPrintedSourceFile>;
	...
	**/
	...
	/**
		Registers one mapping-aware printer result for final artifact emission.

		Why / What / How
		- Reflaxe prints modules lazily after `onCompileEnd`, so the ordinary extra-file stage cannot know
		  generated byte spans in advance.
		- `RustOutputIterator` records each exact output chunk here and emits the aggregate after the final
		  module has printed.
		- The returned snapshot is a defensive copy; `RustSourceMap.encode` owns deterministic filename
		  sorting and same-file chunk aggregation.
	**/
	public function recordSourceMapFile(file:RustPrintedSourceFile):Void {
		if (file == null)
			throw "Cannot record a null Rust source-map file";
		sourceMapFiles.push(file);
	}

	public function sourceMapFileSnapshot():Array<RustPrintedSourceFile> {
		return sourceMapFiles.copy();
	}
